...
const MealsHeader = () => {
  ...
  const handleAddMeal = async () => {
    try {
      setLoading(true);
      const allNutrients = {};
      const response = await axios.post(`${API_ENDPOINT}/meal`, {
        body: {
          name: inputValue,
        },
      ...
      setAllMeals([...meals, newMeal]);
      setMealName(newMeal.mealId);
      setSuccess(true);
    } catch (error) {
  ...
  const AddMealButton = () => {
    <Button size="small" type="primary" onClick={handleAddMeal}>
      {loading ? (
        <>
  ...
  const hanldeAssignMeal = useCallback(async () => {
    setLoading(true);
  ...
  useEffect(() => {
    if (inputValue) {
      const filtered = meals.filter((meal) => meal.name.toLowerCase().includes(inputValue.toLowerCase()));
    ...
    <>
      ...
      {success && (
        <Modal open={success} onOk={() => setSuccess(null)} onCancel={() => setSuccess(null)}>
          <Card>
            <Typography style={{ color: 'green' }}>Operation Executed Successfully</Typography>
          </Card>
        </Modal>
      )}
      <SearchUser page="meals" />

      <div style={{ padding: '0 3px',display:'flex',flexDirection:"row",alignItems:
        "center"
       }}>
        <Select
          showSearch
        ...
        {filteredMeals.length === 0 && (
          <Button style={{zIndex:"10"}} size="small" type="primary" onClick={handleAddMeal}>
            {loading ? (
              <>
                <Loader2 className="animate-spin" size={20} /> wait
              </>
      ...
      <Button size="small" type="primary" onClick={hanldeAssignMeal}>
        {loading ? (
          <>
            <Loader2 className="animate-spin" size={20} /> wait
          </>
